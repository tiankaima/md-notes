...
- 16:52 故障报告
- 2026-04-21 16:15 定位到故障硬盘，拆下等待维修
- 2026-05-27 11:42 厂家维保人员反馈已换新盘，旧数据无法恢复
- 2026-06-08 08:21 维护结束
- 2026-06-08 08:21 通知

    - 新存储方案已重新搭建并上线，原有数据因单盘故障已无法回复，新方案为保证冗余性，总容量调整为 `~42T`，并且拆分了冷、热存储，会提供更好的稳定性。
    - 请注意，拆分后 GPU 节点内仅能访问到使用 SSD 的热存储 `/data/cls1-beegfs/`，登录节点的 `/home/$USER/` 使用 HDD 可作为备份/冷存储，所有计算任务所需文件均需要移动到 `/data/cls1-beegfs/`，避免磁盘 IO 成为瓶颈。
    - 调整了 Slurm 调度相关的 QoS、上限等，如遇问题请反馈
